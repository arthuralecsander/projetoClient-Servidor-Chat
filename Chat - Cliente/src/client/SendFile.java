    ...
    */
    public boolean prepare(String u, String h, int p, MainForm m){
        ...
        try {
            ...
            String format = "CMD_SHARINGSOCKET "+ meuUsuario;
            dos.writeUTF(format);
                ...
                while(!Thread.currentThread().isInterrupted()){
                    ...
                    switch(cmd){
                        ...
                        case "CMD_RECEIVE_FILE_ACCEPT":  // Format: CMD_RECEIVE_FILE_ACCEPT [Message]
                            ...
                            new Thread(new SendingFileThread(socket, arquivo, enviarPara, meuUsuario, SendFile.this)).start();
                            break;
    ...
    private void btnSendFileActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSendFileActionPerformed
        ...
        enviarPara = txtSendTo.getText();
        arquivo = txtFile.getText();
        
        if((enviarPara.length() > 0) && (arquivo.length() > 0)){
            try {
                
                txtFile.setText("");
                String nomeArquivo = getThisFilename(arquivo);//Joao.png = Joao
                String format = "CMD_SEND_FILE_XD "+meuUsuario+" "+enviarPara+" "+nomeArquivo; 
                dos.writeUTF(format);
                updateBtn("Carregando...");
                btnSendFile.setEnabled(false);
            } catch (IOException e) {
                System.out.println(e.getMessage());
            }
        }else{
            JOptionPane.showMessageDialog(this, "Form Imcompleto.!","Error", JOptionPane.ERROR_MESSAGE);
        }
